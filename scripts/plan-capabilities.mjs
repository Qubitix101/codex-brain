...
if (!markdown) {
  console.log(JSON.stringify({
    ...
    next_step: "Fill templates/shared/capability-access-map.template.md after intelligence architecture, agent-engineering, and skill requirements are known; ask the user only for required-now access."
  }, null, 2));
  process.exit(0);
}
...
lines.push("");
lines.push("Create `docs/capability-access-map.md` after `docs/intelligence-architecture-decision.md`, `docs/agent-engineering-audit.md`, and `docs/skill-inventory.md` are complete when relevant. Mark each capability required/mockable/deferred/not applicable, then ask the user only for the required-now credentials, accounts, scopes, or setup information.");
