...
if (!markdown) {
  console.log(JSON.stringify({
    ...
    next_step: "Fill templates/shared/capability-access-map.template.md after intelligence architecture, agent-engineering, skill, Agent OS runtime, agent network, and Agentic Zero Trust requirements are known; ask the user only for required-now access."
  }, null, 2));
  process.exit(0);
}
...
lines.push("");
lines.push("Create `docs/capability-access-map.md` after `docs/intelligence-architecture-decision.md`, `docs/agent-engineering-audit.md`, `docs/skill-inventory.md`, `docs/agent-os-runtime-plan.md`, `docs/agent-network-interop-plan.md`, and `docs/agentic-zero-trust-plan.md` are complete when relevant. Mark each capability required/mockable/deferred/not applicable, then ask the user only for the required-now credentials, accounts, scopes, or setup information.");
