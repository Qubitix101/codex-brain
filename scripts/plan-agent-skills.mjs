...
const result = {
  ...
  next_step: "Create docs/intelligence-architecture-decision.md first when AI architecture is not already selected, then create docs/agent-engineering-audit.md and docs/skill-inventory.md. For L3+ systems, create docs/agent-os-runtime-plan.md before agent network, Agentic Zero Trust, and capability/access mapping."
};
...
lines.push("");
lines.push("Confirm `docs/intelligence-architecture-decision.md` first when AI architecture is not already selected. Then fill `docs/agent-engineering-audit.md` and `docs/skill-inventory.md`. For L3+ systems, fill `docs/agent-os-runtime-plan.md` before agent network, Agentic Zero Trust, and capability/access mapping. Do not generate or install executable skills until T2-T4 trust review requirements are explicit.");
