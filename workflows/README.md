...
- `agent-engineering/` - audit production-agent readiness and map procedural skill candidates before Agent OS runtime, agent network, Agentic Zero Trust, and access planning
- `agent-os-runtime/` - define scheduler, memory manager, tool sandbox, identity, observability, governance, recovery, budgets, agent registry, and human control before agent network, Agentic Zero Trust, and access planning
- `agent-network-interop/` - define agent cards, topology, delegation, MCP versus A2A-style boundaries, context sharing, streaming, traces, recovery, and versioning before Agentic Zero Trust and access planning
- `agentic-zero-trust/` - define non-human identity, JIT credentials, per-action authorization, registry trust, policy enforcement, memory integrity, sandboxing, immutable traces, human controls, adversarial evals, and incident response before access planning
