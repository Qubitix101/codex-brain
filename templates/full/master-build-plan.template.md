...
- [ ] Memory architecture decision complete if durable, customer-specific, cross-session, or cross-agent memory is relevant. Verify: `docs/memory-architecture-decision.md` exists or the Agent OS runtime plan includes explicit per-loop working/semantic/procedural/episodic memory fit, owner, provenance, review, and forgetting rules.
